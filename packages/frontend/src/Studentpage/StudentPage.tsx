...
import rinkbeydai from '../rinkebyDAI.json';

const portis = new Portis('f3b60780-6435-4f71-bf1d-3d2aeba03350', 'rinkeby');
...
export class StudentPage extends React.Component {
  handleClick = () => {
    ...
    // @ts-ignore
    token.methods.totalSupply().call(function (err, res) {
      if (!err) {
        console.log(res);
      } else {
        console.log('error');
  ...
  render() {
    return (
      <div className="AppA">
        ...
        <p className="KhanA"></p>
        <p className="MottoA">
          Ready to take the next step in  your education with Art History?  Earn
          interest while you learn!
        </p>
        <p className="Step01studentA">
          Step 1 Deposit $100 in crypto to  be eligible to gain interest
        </p>
        <p className="DepositmottoA">Deposit $100 for </p>
        <button className="ConfirmA" onClick={this.handleClick}>
          {' '}
          Confirm & Stake
        </button>
        ...
        <p className="Step02studentA">
          Step 2 Complete all modules before  the course period ends
        </p>
